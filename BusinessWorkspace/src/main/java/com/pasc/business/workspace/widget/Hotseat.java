...
public class Hotseat extends LinearLayout implements OnClickListener, View.OnTouchListener {
    private final String TAG = "rick_Print：" + Hotseat.class.getSimpleName();
...
//        void onItemUnClick(int tagIndex);
    ...
    public void addHotseatClickObserver(OnHotseatClickListener onClickListener) {
        ...
        if (onListeners.contains(onClickListener)) {
            PascLog.i(TAG, "onClickListener already exist, ignore");
            return;
        }
    ...
    public int addCellItem(final HotseatDisplayItem displayItem) {
        ...
        if (!isEnabledDisplayItem(displayItem)) {
            PascLog.e(TAG, "info is illegal(already exists), This will be ignored!");
            return -1;
        }
        ...
        PascLog.i(TAG, "addOneBottomButton:" + button.getText());
        mHomeBottomButtons.add(button);
        addView(button);

        return -1;
    }
    public HotseatItemView getCellItem(String actionId){
      if(actionId==null){
      ...
      for(int i=0;i<getChildCount();i++){
         ...
         if(actionId.equals(hotseatItemView.getComponentName().getClassId())){
             return hotseatItemView;
         }
      }
      return null;
    }
    public HotseatItemView getCellItem(int index){
      if(index<0 && index>getChildCount()-1){
        throw new IndexOutOfBoundsException("index out or range");
      }
    return (HotseatItemView) getChildAt(index);
  }
...
//            ((HotseatItemView) view).setHighlight(0);
            ...
            PascLog.i(TAG, "onClick view=" + view);
        }
    }
    ...
    private void setFocus(final HotseatItemView button) {
        PascLog.i(TAG, "call setFocus HomeBottomButton");
        if (null == button) {
            Exception here = new Exception("call setFouce on null object");
            here.fillInStackTrace();
            PascLog.i(TAG, "call setFouce on null object", here);
            return;
        }
        ...
        if (!matched) {
            mFoucsButton = null;
            PascLog.i(TAG, "setFouce:HomeBottomButton - classId is " + button.getClassId()
    ...
    private void printHomeBottomButtonsInfo() {
        ...
        for (HotseatItemView button : mHomeBottomButtons) {
            PascLog.i(TAG, "[" + index + "] classId:" + button.getClassId() + " type is " + button.getComponentType());
        }
    }
